...
public class ShapeGlitterRenderManager {
    private static final List<Runnable> RENDER_TASKS = new ArrayList<>();
    private static final Matrix4f savedProjection = new Matrix4f();
    private static final Matrix4f savedModelView = new Matrix4f();

    public static void queueRender(Runnable task) {
    ...
    public static void renderAllAndClear() {
        ...
        for (Runnable task : RENDER_TASKS) {
            task.run();
        }
